...
class GFUNet(SegmentationNetwork):
    ...
    def __init__(
        ...
        self.norm1 = nn.LayerNorm([embed_dim[2], 7, 7])
        self.norm2 = nn.LayerNorm([embed_dim[1], 14, 14])
        self.norm3 = nn.LayerNorm([embed_dim[0], 28, 28])
#         self.norm4 = nn.LayerNorm([embed_dim[0], 56, 56])
...
#         self.decoder5 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
        ...
        norm_name = "instance"
        self.res_block1 = UnetResBlock(2, 512, 256, kernel_size=3, stride=1, norm_name = "instance",)
        self.res_block2 = UnetResBlock(2, 256, 128, kernel_size=3, stride=1, norm_name = "instance",)
        self.res_block3 = UnetResBlock(2, 128, 64, kernel_size=3, stride=1, norm_name = "instance",)
        self.res_block4 = UnetResBlock(2, 64, 3, kernel_size=3, stride=1, norm_name = "instance",)
...
#         x = self.norm(x)# .mean(1)
    ...
    def backward_features(self, x):# add skip connection
        B = x.shape[0]
        for i in range(4):
            B, C, H, W = x.shape
...
#     def bottelneck(self, x):



# torch.Size([8, 3, 224, 224])

# torch.Size([8, 3136, 64embedding]) 56 56    
# torch.Size([8, 64, 56, 56])

# torch.Size([8, 784, 128]) 28 28 
# torch.Size([8, 128, 28, 28])

# torch.Size([8, 196, 256]) 14 14 
# torch.Size([8, 256, 14, 14])

# torch.Size([8, 49, 512]) 7 7 
# torch.Size([8, 512, 7, 7])
    def forward(self, x):
        ...
        # up self.layer_norm([self.embed_dim[3], H, W])
        
        # 1
        x = F.relu(F.interpolate(self.norm1(self.decoder1(x)),scale_factor=(2,2),mode ='bilinear'))
        # torch.Size([8, 256, 14, 14])
        print(x.shape)
...
#         x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        ...
        # 2
        x = F.relu(F.interpolate(self.norm2(self.decoder2(x)),scale_factor=(2,2),mode ='bilinear'))
        print(x.shape)
...
#         x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        ...
        x = F.relu(F.interpolate(self.norm3(self.decoder3(x)),scale_factor=(2,2),mode ='bilinear'))
        print(x.shape)
...
#         x = F.relu(F.interpolate(self.norm4(self.decoder4(x)),scale_factor=(2,2),mode ='bilinear'))
#         print(x.shape)
...
# embed_dim根据网络size决定
class GFUNet_V(SegmentationNetwork):
...
#         self.norm4 = nn.LayerNorm([embed_dim[0], 56, 56])
        ...
        self.decoder2 = nn.Conv2d(256, 128, 3, stride=1, padding=1)  
        self.decoder3 = nn.Conv2d(128, 64, 3, stride=1, padding=1) 
        self.decoder4 = nn.Conv2d(64, 3, 3, stride=1, padding=1)
#         self.decoder5 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
...
def create_GFNet_T224(deploy=False):
    return GFNet(channels=(64, 128, 256, 512), hs=(56,28,14,7), ws=(56,28,14,7),
                      num_blocks=(2,2,6,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,16,128),
                     deploy=deploy)
def create_GFNet_T256(deploy=False):
    return GFNet(channels=(64, 128, 256, 512), hs=(64,32,16,8), ws=(64,32,16,8),
                      num_blocks=(2,2,6,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,16,128),
                     deploy=deploy)
def create_GFNet_B224(deploy=False):
    return GFNet(channels=(96, 192, 384, 768), hs=(56,28,14,7), ws=(56,28,14,7),
                      num_blocks=(2,2,12,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,32,128),
                     deploy=deploy)
def create_GFNet_B256(deploy=False):
    return GFNet(channels=(96, 192, 384, 768), hs=(64,32,16,8), ws=(64,32,16,8),
                      num_blocks=(2,2,12,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,32,128),
                     deploy=deploy)
def create_GFNet_D256(deploy=False):
    return GFNet(channels=(80, 160, 320, 640), hs=(64,32,16,8), ws=(64,32,16,8),
                      num_blocks=(2,2,18,2), reparam_conv_k=(1, 3), sharesets_nums=(1,4,16,128),
                     deploy=deploy)
def create_GFNet_L256(deploy=False):
    return GFNet(channels=(96, 192, 384, 768), hs=(64,32,16,8), ws=(64,32,16,8),
