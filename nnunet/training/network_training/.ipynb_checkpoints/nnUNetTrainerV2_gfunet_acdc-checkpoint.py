...
from nnunet.network_architecture.UNext import UNext
from nnunet.network_architecture.GFUNet import GFUNet
...
class nnUNetTrainerV2_gfunet_acdc(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        if self.model_size == 'Tiny':
            ...
            if self.pretrain:
                self.pre_trained_weight = torch.load("../../../../weight/convnext_small_22k_224.pth",map_location='cpu')
            
        if self.model_size=='Base':
            self.embedding_dim = 128
            self.num_heads = [4,8,16,32]
            if self.pretrain:
                self.pre_trained_weight = torch.load("../../../../weight/convnext_base_22k_224.pth",map_location='cpu')

        if self.model_size=='Large':
            self.embedding_dim = 192
            self.num_heads = [6,12,24,48]
            if self.pretrain:
                self.pre_trained_weight = torch.load("../../../../weight/convnext_large_22k_224.pth",map_location='cpu')
