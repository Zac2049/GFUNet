...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {},weight_ce=0.8, weight_dice=1.2)
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            dl_tr = DataLoader2D_unet2022(self.dataset_tr, int(self.data_aug_params['num_threads']),batch_size=self.batch_size,patch_size=self.basic_generator_patch_size,final_patch_size=self.patch_size)
            dl_val = DataLoader2D_unet2022(self.dataset_val, int(self.data_aug_params['num_threads'])//2,batch_size=self.batch_size,patch_size=self.patch_size,final_patch_size=self.patch_size)
            
           
        self.print_to_log_file("Batch size: %s" % (str(self.batch_size)),also_print_to_console=False)
        self.print_to_log_file("Patch size: %s" % (str(self.patch_size)),also_print_to_console=False)
        return dl_tr, dl_val
    ...
    def finish_online_evaluation(self):
        ...
        self.print_to_log_file("Average foreground Dice:", str(self.all_val_eval_metrics[-1]))

        self.print_to_log_file("(interpret this as an estimate for the Dice of the different classes. This is not "
                               "exact.)")
        #self.print_to_log_file('mean_data:',str(np.mean(self.mean_data)))
        self.online_eval_foreground_dc = []
        self.online_eval_tp = []
        self.online_eval_fp = []
        self.online_eval_fn = []
